...
class Akismet_Admin {
	...
	public static function comment_row_action( $a, $comment ) {
		...
		if ( $akismet_result ) {
			...
			foreach ( $a as $k => $item ) {
				...
				if (
					...
					$b['history'] = '<a href="comments.php?action=editcomment&amp;c='.$comment->comment_ID.'#akismet-status" title="'. esc_attr__( 'View comment history' , 'akismet') . '"> '. esc_html__('History', 'akismet') . '</a>';
				}
			}
		...
		if ( $desc )
			echo '<span class="akismet-status" commentid="'.$comment->comment_ID.'"><a href="comments.php?action=editcomment&amp;c='.$comment->comment_ID.'#akismet-status" title="' . esc_attr__( 'View comment history' , 'akismet') . '">'.esc_html( $desc ).'</a></span>';
